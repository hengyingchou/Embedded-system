...
void init_UART(void){
  ...
  RCGCUART |= (1<<2); // define UART regrister
  RCGCGPIO |= (1<<0); // configure GPIO port
  GPIOAFSEL_A = (1<<6)|(1<<7); // configure AFSEL for pin0 and pin1
  GPIOPCTL_A = (1<<24)|(1<<28); // GPIO Port control for 0 and 4 positions
  GPIODEN_A = (1<<6) | (1<<7); // GPIO Digital Enable
  UART2_CTL &= ~(1<<0); // UART Control 
  UART2_IBRD = 781;  //  Integer Baud-Rate Divisor
  UART2_FBRD = 16;  // Fractional Baud-Rate Divisor
  UART2_LCRH = (0x3<<5); // Line Control
  UART2_CC = 0x0; // Clock Configuration
  UART2_DR = 0x00000; // UART Data 
  UART2_CTL = (1<<0)|(1<<8)|(1<<9); // enable UART control
}
